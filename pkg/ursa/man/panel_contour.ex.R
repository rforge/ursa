invisible({
  ...
  # ref <- colorize(a,breakvalue=refval,palname="Purples")
   ref <- colorize(a,breakvalue=refval,pal.rich=45,pal.r=0)
  # p1 <- colorize(a,breakvalue=val,palname="Oranges")
   p1 <- colorize(a,breakvalue=val,pal.rich=135,pal.r=0)
  # p2 <- colorize(a,value=val,palname="Blues",inv=TRUE)
   p2 <- colorize(a,breakvalue=val,pal.rich=-15,pal.r=0)
   ...
   if (TRUE) {
      if (TRUE) {
        ...
        # ct2 <- panel_contour(p2,"colored line",palname="Greens",lwd=15,lwd.bg=0)
         ct2 <- panel_contour(p2,"colored line",pal.rich=240,pal.r=0,lwd.fg=15,lwd.bg=0)
   ...
   if (TRUE) {
     ...
     # ct0 <- panel_contour(a,"color",value=val,palname="Greens",lwd=11,lwd.bg=12)
      ct0 <- panel_contour(a,"color",value=val,pal.rich=240,pal.r=0,lwd=11,lwd.bg=12)
      panel_contour(a)
