...
'panel_annotation' <- function(...)
{
   ...
   if (!annotation)
      return(NULL)
   label <- .getPrm(arglist,name="($|label|text)",kwd=kwd,class=rev(c("expression","character"))
...
'.panel_annotation' <- function(label=expression()#Omega^alpha)
  ...
  # scale <- getOption("ursaPngScale")
   ...
   coord[2] <- minx+pos[1]*(maxx-minx)+0.5*width
   coord[3] <- miny+pos[2]*(maxy-miny)-0.5*height
   coord[4] <- miny+pos[2]*(maxy-miny)+0.5*height
   if (coord[1]<minx)
   {
      coord[1] <- minx-ifelse(isE,width/3.5,0)
      coord[2] <- coord[1]+width
   }
   if (coord[2]>maxx)
   {
      coord[2] <- maxx+ifelse(isE,width/3.5,0)
      coord[1] <- coord[2]-width
   }
   if (coord[3]<miny)
   {
      coord[3] <- miny
      coord[4] <- coord[3]+height
   }
   if (coord[4]>maxy)
   {
      coord[4] <- maxy-ifelse(isE,height,0)
      coord[3] <- coord[4]-height
   }
  # print(c(scale=scale,cex=mycex))
   x <- 0.5*(coord[1]+coord[2])
   y <- 0.5*(coord[3]+coord[4])
   x <- x+(adjust-0.5)*(coord[2]-coord[1])
   if ((!is.na(fill))&&(is.character(fill))) ##(isCaption)
      rect(coord[1],coord[3],coord[2],coord[4],border="transparent",col=fill)
